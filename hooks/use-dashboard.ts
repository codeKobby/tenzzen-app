...
export function useUserStats() {
  ...
  if (!user?.id) {
    return { userStats: { total_learning_hours: 0, courses_in_progress: 0, courses_completed: 0, projects_submitted: 0 }, loading: false };
  }

  if (!stats) return { userStats: { total_learning_hours: 0, courses_in_progress: 0, courses_completed: 0, projects_submitted: 0 }, loading: true };
  ...
  const totalLearningHours = weeklyActivity.reduce((a: number, b: number) => a + (Number(b) || 0), 0);
...
export function useRecentCourses(limit: number = 5) {
  ...
  const mapped = (courses || [])
    .filter(Boolean)
    .map((course: any) => ({
      id: course._id || course._id?.toString?.() || course.id || String(course._id || ''),
      title: course.title || 'Untitled',
      description: course.description || course.summary || '',
      progress: course.enrollment?.progress ?? course.enrollment_progress ?? 0,
      thumbnail: course.thumbnail || course.sourceUrl || course.source_url || '',
      sections: course.modules || course.sections || [],
      completed_lessons: course.enrollment?.completedLessonIds || course.completed_lessons || [],
      last_accessed_at: course.enrollment?.lastAccessedAt || course.last_accessed_at || new Date().toISOString(),
...
export function useRecentActivities(limit: number = 10) {
  ...
  const activities = (stats.recentActivities || [])
    ...
    .map((activity: any, idx: number) => ({
      ...
      title: activity.metadata?.title || getActivityTitle(activity) || activity.type,
      timestamp: activity.createdAt || activity.created_at || new Date().toISOString(),
      metadata: activity.metadata || {},
      course_name: activity.metadata?.courseName || activity.course_name || ''
    } as LearningActivity));
...
export function useLearningTrends(period: string = '30d') {
  const { user } = useUser();
  const days = period === 'week' || period === '7d' ? 7 : period === '30d' ? 30 : 90;
  const activity = useQuery(api.dashboard.getLearningActivity, user?.id ? { userId: user.id, days } : 'skip');

  if (!user?.id) return { trends: { weeklyActivity: [0, 0, 0, 0, 0, 0, 0] }, loading: false };
  if (!activity) return { trends: { weeklyActivity: [0, 0, 0, 0, 0, 0, 0] }, loading: true };
...
function getActivityTitle(activity: any): string {
  ...
  switch (type) {
    ...
    case 'login':
      return 'Logged in';
    default:
      return activity.metadata?.title || activity.metadata?.action || 'Activity';
  }
}
