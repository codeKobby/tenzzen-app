...
export function useUserActivity(options: UseUserActivityOptions = {}) {
      ...
      if (response.ok) {
        ...
        if (data.success && data.activity && data.activity.date === today) {
          ...
          if (data.activity.minutes > activeTime) {
            ...
            localStorage.setItem(`user_${user.id}_active_time`, JSON.stringify({
              date: today,
  ...
  useEffect(() => {
    ...
    const updateActiveTime = () => {
      ...
      if (isVisibleRef.current && !isIdleRef.current && activeTabRef.current === tabIdRef.current) {
  ...
  }, [syncInterval, isUserLoaded, isSignedIn, user?.id, activeTime]);
  
  // Sync to database when unmounting
  useEffect(() => {
    return () => {
      if (isUserLoaded && isSignedIn && user?.id && activeTime > 0) {
        syncActivityToDatabase();
      }
    };
  }, [isUserLoaded, isSignedIn, user?.id, activeTime]);
